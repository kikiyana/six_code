...
def isValidEncoding(s):
    ...
    right_queue = CircularQueue(len(s) + 1)

    for char in s:
        if char in "([{":
            left_queue.enqueue(char)
        elif char in ")]}":
            right_queue.enqueue(char)
            if left_queue.front == left_queue.rear:
                return False
            left_char = left_queue.dequeue()
            if (char == ')' and left_char != '(') or (char == ']' and left_char != '[') or (char == '}' and left_char != '{'):
                return False
    
    return left_queue.front == left_queue.rear and right_queue.front == right_queue.rear
