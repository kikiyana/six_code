...
# 训练
def train(model, optimizer, train_x, train_y, n_way, n_support, n_query, max_epoch, epoch_size, save_path=None):
...
def test(model, test_x, test_y, n_way, n_support, n_query, test_episode):
    ...
    for episode in tqdm(range(test_episode)):
        ...
        for idx, pred in enumerate(preds):
            conf_mat += np.bincount(pred, minlength=n_way)
...
if __name__ == '__main__':
    ...
    train_x, test_x, train_y, test_y = train_test_split(csi, trainy, test_size=0.2, random_state=20,
    ...
    model_out_name = 'models/model_mix_test' + '.pt'
    ...
    # 训练
    train(model, optimizer, train_x, train_y, n_way, n_support, n_query, max_epoch, epoch_size, model_out_name)
