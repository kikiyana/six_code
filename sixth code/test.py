...
def train(model, train_loader, criterion, optimizer, num_epochs=20, device='cuda'):
    model.to(device)
    model.train()
    for epoch in range(num_epochs):
        ...
        for data in train_loader:
            ...
            # 对于孪生网络或多类孪生网络
            outputs = model(inputs1, inputs2)  # 直接传递成对的输入
...
if __name__ == "__main__":
    ...
    train_loader, test_loader = create_dataloader(path1, batch_size=args.batch, num_workers=1)
